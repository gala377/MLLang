...
async def run_async(args):
    ...
    with tempfile.TemporaryDirectory() as build_dir:
        ...
        if not (fnk_binary.exists() and fnk_binary.is_file()):
            raise RuntimeError(f"Funk binary {fnk_binary} does not exist")
        await run_tests_async(fnk_binary, test_files)
...
async def run_tests_async(binary: Path, tests: list[Path]):
    failed = []
    for test in tests:
        print(f"Running test {strip_tests_dir(test)}... ", end='')
        test_path, expected = await parse_test_file_async(trio.Path(test.absolute()))
        test_output = expected is not None
        if test_output:
            test_file = test_path
            test_path = test_file.name
...
async def parse_test_file_async(test: trio.Path):
    async with await test.open() as tf:
        if (l := await tf.readline()) != "@EXPECTED\n":
            return test, None
        expected = []
        async for line in tf :
            if line == "@SOURCE\n":
                break
            expected.append(line)
        test_source = trio.wrap_file(tempfile.NamedTemporaryFile())
        async for line in tf:
            await test_source.write(bytes(line, 'utf-8'))
        await test_source.seek(0)
        expected = list(filter(bool, map(str.rstrip, expected)))
        return test_source, expected
